<?php
class FifthPaymentInInstallmentsDAO
{
    public function create(PaymentInInstallments $paymentInInstallments)
    {
        try {
            $sql = "INSERT INTO tbl_fifth_installment (
    ...
    public function findByClientId($idClient) {
        try {
            $sql = "SELECT * FROM tbl_fifth_installment WHERE idclient = :idclient";
            $p_sql = Conexao::getConexao()->prepare($sql);
    ...
    public function listOpenInstallment() {
        try {
            ...
            $sql = "SELECT * FROM tbl_fifth_installment WHERE installment_status = 'Em Aberto' AND installment_date <= '$currentDate' ORDER BY idclient ASC";
