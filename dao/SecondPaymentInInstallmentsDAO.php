<?php
class SecondPaymentInInstallmentsDAO
{
    public function create(PaymentInInstallments $paymentInInstallments)
    {
        try {
            $sql = "INSERT INTO tbl_second_installment (
    ...
    {
        try {
            $sql = "UPDATE tbl_second_installment SET
    ...
    public function findByClientId($idClient) {
        try {
            $sql = "SELECT * FROM tbl_second_installment WHERE idclient = :idclient";
            $p_sql = Conexao::getConexao()->prepare($sql);
    ...
    public function listOpenInstallment() {
        try {
            ...
            $sql = "SELECT * FROM tbl_second_installment WHERE installment_status = 'Em Aberto' AND installment_date <= '$currentDate' ORDER BY idclient ASC";
