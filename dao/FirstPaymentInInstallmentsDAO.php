...
class FirstPaymentInInstallmentsDAO
{
    ...
    {
        try {
            $sql = "UPDATE tbl_first_installment SET
    ...
    public function findByClientId($idClient) {
        try {
            $sql = "SELECT * FROM tbl_first_installment WHERE idclient = :idclient";
            $p_sql = Conexao::getConexao()->prepare($sql);
    ...
    public function listOpenInstallment() {
        try {
            ...
            $sql = "SELECT * FROM tbl_first_installment WHERE installment_status = 'Em Aberto' AND installment_date <= '$currentDate' ORDER BY idclient ASC";
